...
export function TicketDetailPage({ isDark, onToggleTheme }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[radial-gradient(circle_at_top_left,#12314f_0%,#06080d_58%)] text-slate-100">
      <header className="flex flex-wrap items-center gap-3 border-b border-slate-700/70 bg-slate-900/80 px-4 py-3 backdrop-blur sm:px-7">
        <button
          onClick={() => navigate(backPath)}
          className="rounded-md border border-slate-600 bg-slate-800 px-3 py-1.5 text-sm text-slate-100 transition hover:border-blue-500"
        >
          ← Back
        </button>
        ...
        <div>
          ...
          <span className="text-xs text-slate-400">#{ticketId}</span>
        </div>
      ...
      <main className="flex flex-1 flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto px-4 py-5 sm:px-7">
          <div className="flex flex-col gap-3">
              ...
              messages.map((msg) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`mt-1 flex gap-2 text-[11px] text-slate-400 ${msg.responseActor === "ADMIN" ? "justify-end" : "justify-start"}`}
                    >
                      <span
            ...
            {isOtherTyping && (
              <div className="max-w-[78%] self-end rounded-lg border border-slate-700 bg-slate-900/80 px-3 py-2 text-xs italic text-slate-400 sm:max-w-[60%]">
        ...
        <form
          onSubmit={handleSend}
          className="border-t border-slate-700/70 bg-slate-900/80 px-4 py-3 sm:px-7"
        >
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              {showMentionMenu && (
                <div className="absolute inset-x-0 bottom-12 z-10 rounded-2xl border border-slate-700 bg-slate-900 p-2 shadow-xl">
                  <button
                    type="button"
                    onClick={handleMentionSelect}
                    className="flex w-full items-center gap-3 rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-left transition hover:border-blue-500"
                  >
              ...
              <input
                ...
                className="w-full rounded-full border border-slate-600 bg-slate-950 px-4 py-2.5 text-sm text-blue-300 outline-none transition placeholder:text-slate-500 focus:border-blue-500"
              />
            </div>
